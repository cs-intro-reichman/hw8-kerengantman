...
public class Network {
    ...
    public String recommendWhoToFollow(String name) {
        ...
        int current = 0;
        for (int i = 0; i < userCount; i++) {
            current = user.countMutual(users[i]);
            if (users[i].getName().equals(name)) {
                continue;
            }
            if (maxfollower < current) {
                maxfollower = current;
                mostrecommend = users[i];
            }
        }
        return mostrecommend.getName();
    }
    ...
    public String mostPopularUser() {
        ...
        for (int i = 0; i < userCount; i++) {
            ...
            if (currentCount > maxfCount) {
                maxfCount = currentCount;
